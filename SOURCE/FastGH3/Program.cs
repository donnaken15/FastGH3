...
class Program
{
	...
	static byte[] paknew = new byte[0xB0], paknewP1 = {
				...
				0xE1, 0x53, 0x10, 0xCD, 0xE1, 0x53, 0x10, 0xCD, 0x4C, 0x1E, 0x75, 0x69,
				0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xB5, 0x24, 0x56, 0x5F,
	...
	static string folder, dataf = "\\files\\", pakf = dataf + "PAK\\",
		music = dataf + "MUSIC\\", song = "song", title = "FastGHWii";
	...
	static void Main(string[] args)
	{
		try
		{
				...
				if (multiinstcheck.Length > 1)
					foreach (Process fgh3 in multiinstcheck)
					{
							...
							NormalizePath(Application.ExecutablePath))
						{
							...
							{
								MessageBox.Show("FastGHWii Launcher is already running!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
			...
			GH3EXEPath = NormalizePath(folder + "\\play.bat");
			...
			if (args.Length > 0)
			{
				...
				if (args[0] != "-settings")
					if (writefile)
					{
						...
						if (newfile)
						{
							launcherlog.WriteLine("FastGHWii Launcher LogTM(C)(R)Allrightsreserved\n--------------------------------");
							try
							{
				...
				else if (File.Exists(args[0]))
				{
					...
					if (Path.GetFileName(args[0]).EndsWith(chartext) || Path.GetFileName(args[0]).EndsWith(midext))
					{
						...
						if (cacheEnabled)
						{
							...
							cacheList = Directory.GetFiles(folder + dataf + "CACHE");
							...
							do
							{
								...
								if (audiolost == DialogResult.No || searchaudioresult == DialogResult.Cancel || !File.Exists(searchaudio.FileName))
								{
									...
									if (playsilent == DialogResult.Yes)
									{
										...
										audiostreams[0] = folder + music + "\\TOOLS\\blank.wav";
									}
								}
							}
						...
						for (int i = 0; i < 3; i++)
							if (!File.Exists(audiostreams[i]))
							{
								audiostreams[i] = folder + music + "\\TOOLS\\blank.wav";
							}
						//im stupid
						...
						if (!audCache)
						{
							...
							if (notjust3trax)
							{
								...
								audiostreams[0] = folder + music + "\\TOOLS\\fastgh3_song\\fastgh3_song.wav";
							...
							{
								Directory.CreateDirectory(folder + music + "\\TOOLS\\fastgh3_preview");
								File.Copy(folder + music + "\\TOOLS\\blankA.wav", folder + music + "\\TOOLS\\fastgh3_preview\\fastgh3_preview.wav", true);
								string[] fsbnames = { "song", "guitar", "rhythm" };
								for (int i = 0; i < fsbbuild2.Length; i++)
								{
									Directory.CreateDirectory(folder + music + "\\TOOLS\\fastgh3_" + fsbnames[i]);
									fsbbuild2[i] = new Process();
									fsbbuild2[i].StartInfo = new ProcessStartInfo()
									{
										FileName = CMDpath,
										Arguments = "/c " + ((folder + music + "\\TOOLS\\c128ks.bat").EncloseWithQuoteMarks() + " " + audiostreams[i].EncloseWithQuoteMarks() + " \"" + folder + music + "\\TOOLS\\fastgh3_"+ fsbnames[i] + "\\fastgh3_" + fsbnames[i] + ".wav\"").EncloseWithQuoteMarks(),
										CreateNoWindow = true,
							...
							if (!MTFSB)
							{
								...
								(folder + music + "\\TOOLS\\blank.wav").EncloseWithQuoteMarks() + ' ' + (folder + music + "\\fastgh3").EncloseWithQuoteMarks()).EncloseWithQuoteMarks();
							...
							else
							{
								...
								fsbbuild3.StartInfo.Arguments = "/c " + ((folder + music + "\\TOOLS\\fsbbuildnoenc.bat").EncloseWithQuoteMarks() + ' ' +
									(folder + music + "\\fastgh3").EncloseWithQuoteMarks()).EncloseWithQuoteMarks();
							}
						}
						else
						{
							...
							{
								File.Copy(
									folder + "\\files\\CACHE\\" + audhash.ToString("X16") + ".DAT",
									folder + "\\files\\MUSIC\\fastgh3.dat.ngc", true);
								File.Copy(
									folder + "\\files\\CACHE\\" + audhash.ToString("X16") + ".WAD",
									folder + "\\files\\MUSIC\\fastgh3.wad.ngc", true);
							}
							catch (IOException e)
							{
								...
								File.Delete(folder + "\\files\\MUSIC\\fastgh3.dat.ngc");
								File.Delete(folder + "\\files\\MUSIC\\fastgh3.wad.ngc");
								File.Copy(
									folder + "\\files\\CACHE\\" + audhash.ToString("X16") + ".DAT",
									folder + "\\files\\MUSIC\\fastgh3.dat.ngc", true);
								File.Copy(
						...
						if (!chartCache)
						{
							...
							File.WriteAllBytes(folder + pakf + "song.pak.ngc", paknew);
							verboseline("Creating PakFormat and PakEditor from song.pak", chartConvColor);
							print("Opening song pak.", chartConvColor);
							PakFormat pakformat = new PakFormat(folder + pakf + "song.pak.ngc", "", "", PakFormatType.Wii);
							PakEditor buildsong;
							...
							if (cacheEnabled)
							{
								...
								File.Copy(
									folder + "\\files\\PAK\\song.pak.ngc",
									folder + "\\files\\CACHE\\" + charthash.ToString("X16"), true);
						...
						else
						{
							...
							File.Copy(
								folder + "\\files\\CACHE\\" + cacheidStr,
								folder + "\\files\\PAK\\song.pak.ngc", true);
							File.Copy(args[0], paksongmid, true);
							mid2chart.Start();
							mid2chart.WaitForExit();
							if (ischart)
							...
							if (!MTFSB)
							{
								if (!fsbbuild.HasExited)
								{
									...
									if (cacheEnabled)
									{
										...
										File.Copy(
											folder + "\\files\\MUSIC\\fastgh3.dat.ngc",
											folder + "\\files\\CACHE\\" + audhash.ToString("X16") +".DAT", true);
										File.Copy(
											folder + "\\files\\MUSIC\\fastgh3.wad.ngc",
											folder + "\\files\\CACHE\\" + audhash.ToString("X16") +".WAD", true);
						...
						args[0].EndsWith(".7z") || args[0].EndsWith(".rar")))
					{
						...
						bool gotWad = false, gotDat = false;
						if (fspcache && Directory.Exists(tmpf) && cacheEnabled)
						{
							...
							foreach (string f in Directory.GetFiles(tmpf, "*.*", SearchOption.AllDirectories))
							{
									...
									f.EndsWith(".pak.ngc"))
								{
									...
									disallowGameStartup();
									File.Delete(folder + pakf + "song.pak.ngc");
									File.Copy(f, folder + pakf + "song.pak.ngc", true);
								...
								if (f.EndsWith(".wad") ||
									f.EndsWith(".wad.ngc"))
								{
									verboseline("Found .wad", FSPcolor);
									disallowGameStartup();
									File.Delete(folder + music + "fastgh3.wad.ngc");
									File.Copy(f, folder + music + "fastgh3.wad.ngc", true);
									gotWad = true;
								}
								if (f.EndsWith(".dat") ||
									f.EndsWith(".dat.ngc"))
								{
									verboseline("Found .dat", FSPcolor);
									disallowGameStartup();
									File.Delete(folder + music + "fastgh3.dat.ngc");
									File.Copy(f, folder + music + "fastgh3.dat.ngc", true);
									gotDat = true;
								}
								if (f.EndsWith(".chart") ||
									f.EndsWith(".mid"))
									...
									foreach (ZipEntry data in file)
									{
										try
										{
											...
											if (data.FileName.EndsWith(".pak") ||
												data.FileName.EndsWith(".pak.ngc"))
											{
												disallowGameStartup();
												...
												File.Delete(folder + pakf + "song.pak.ngc");
												File.Copy(tmpf + data.FileName, folder + pakf + "song.pak.ngc", true);
												multichartcheck.Add(data.FileName);
												compiled = true;
											}
											if (data.FileName.EndsWith(".wad") ||
												data.FileName.EndsWith(".wad.ngc"))
											{
												disallowGameStartup();
												verboseline("Found .wad, extracting...", FSPcolor);
												data.Extract(tmpf);
												File.Delete(folder + music + "fastgh3.wad.ngc");
												File.Copy(tmpf + data.FileName, folder + music + "fastgh3.wad.ngc", true);
												gotWad = true;
											}
											if (data.FileName.EndsWith(".dat") ||
												data.FileName.EndsWith(".dat.ngc"))
											{
												disallowGameStartup();
												verboseline("Found .dat, extracting...", FSPcolor);
												data.Extract(tmpf);
												File.Delete(folder + music + "fastgh3.dat.ngc");
												File.Copy(tmpf + data.FileName, folder + music + "fastgh3.dat.ngc", true);
												gotDat = true;
											}
								...
								if (got7Z || gotWRAR)
								{
									...
									foreach (string f in Directory.GetFiles(tmpf, "*.*", SearchOption.AllDirectories))
									{
											...
											f.EndsWith(".pak.ngc"))
										{
											...
											// do something with this moving when multiple files exist or whatever
											File.Delete(folder + pakf + "song.pak.ngc");
											File.Move(f, folder + pakf + "song.pak.ngc");
											multichartcheck.Add(f);
											compiled = true;
										}
										if (f.EndsWith(".wad") ||
											f.EndsWith(".wad.ngc"))
										{
											...
											verboseline("Found .wad", FSPcolor);
											File.Delete(folder + music + "fastgh3.wad.ngc");
											File.Move(f, folder + music + "fastgh3.wad.ngc");
											gotWad = true;
										}
										if (f.EndsWith(".dat") ||
											f.EndsWith(".dat.ngc"))
										{
											...
											verboseline("Found .dat", FSPcolor);
											File.Delete(folder + music + "fastgh3.dat.ngc");
											File.Move(f, folder + music + "fastgh3.dat.ngc");
											gotDat = true;
										}
										if (f.EndsWith(".chart") ||
											f.EndsWith(".mid"))
						...
						if ((!gotWad || !gotDat) && compiled)
						{
							MessageBox.Show("Missing DAT/WAD", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
							if (writefile && launcherlog != null)
								launcherlog.Close();
							settings.SetKeyValue("Misc", "FinishedLog", "1");
							settings.Save(folder + "settings.ini");
